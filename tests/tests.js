...
const pushToDepsArray = (invokedComponentName, componentName) => {
  ...
  } else if(!componentsReigistry[componentName].includes(invokedComponentName)) {
      ...
      .then((files) => {
        ...
        return Promise.all(promiseArray).then(() => {
          ...
          while (resolvedDeps.length != Object.keys(componentsReigistry).length) {
            const deps = Object
              .keys(componentsReigistry)
              .filter((key) => componentsReigistry[key].length === index);

            for (const dep of deps) {
              Object.keys(componentsReigistry)
                .forEach((key) => {
